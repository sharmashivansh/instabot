...
#defining a self imformation of instabot
def self_info():
...
def get_own_post():
    ...
    if own_media['meta']['code'] == 200:
        ...
        print 'Status code other than 200 received!'
#defining a uername post
def get_user_post(insta_username):
    user_id = get_user_id(insta_username)
    if user_id == None:
        print 'User does not exist!'
        exit()
    request_url = (BASE_URL + 'users/%s/media/recent/?access_token=%s') % (user_id)
    print 'GET request url : %s' % (request_url)
    user_media = requests.get(request_url).json()
    print user_media['data'][0]['images']
    if user_media['meta']['code'] == 200:
        if len(user_media['data']):
        ...
        print 'Status code other than 200 received!'
#defining a like a post of user
        ...
        if len(like_list['data']):
            ...
            for users in like_list['data']:
                if users in like_list['data']:
                    ...
                    print colored("user does nt have any post", 'red')
    else:
        print colored('\n\t status coe otyher than 200 recevied.\n','red')

#defining the comment box
def get_comment_list(insta_username):
    ...
    if comment_list['meta']['code'] == 200:
        if len(comment_list['data']):
            position = 1
            print colored("\n\t list of people who comment your recent post", 'blue')
            for _ in range(len(comment_list['data'])):
                if comment_list['data'][position-1]['text']:
                    print'\n\t',colored(comment_list['data'][position-1]['from']['username'],'green')\
                                +colored('said:','green')+colored(comment_list['data'][position-1]['text'],'blue')
